# Caddy 2.11.4's official binary predates fixed Go/x packages published later
# in 2026. Build the signed-release source at an immutable commit with those
# exact patched modules and a patched Go toolchain, then copy only the static
# binary into a current, upgraded Alpine runtime. Keep this block byte-for-byte
# aligned with relay.Dockerfile so BuildKit can share its cache.
...
COPY --chown=10001:10001 deploy/internal-labs/Caddyfile /etc/caddy/Caddyfile
